...
import type { Conversation, Memory, ChatMessage } from "@nebula/core/types/chat";
import { generateId } from "@nebula/core/utils";
import { createSqliteStorage } from "@/lib/storage";
...
interface LunaState {
  ...
  memories: Memory[];
  actionResults: Record<string, import("@nebula/core/constellation-registry").ActionResult[]>;
  ...
  setActionResults: (
    messageId: string,
    results: import("@nebula/core/constellation-registry").ActionResult[],
  ) => void;
  setMessageSources: (
    conversationId: string,
    messageId: string,
    sources: import("@nebula/core/types/search").SearchSource[],
  ) => void;
...
export const useLunaStore = create<LunaState>()(
  persist(
    ...
    {
      name: "nebula-luna",
      storage: createJSONStorage(() => createSqliteStorage("nebula-luna")),
      partialize: (s) => ({
